...
- **Granular Lesson Tree & Syllabus Alignment:** Maps 56 unique lessons for CIE tracks and 21 lessons (across 6 units) for Edexcel, aligned 100% with the official Pearson Edexcel IAL syllabus (Topics 11 to 20). Fully completed interactive curricula for:
  ...
  - **Edexcel Unit 5 Topic 20 / Unit 4 Topic 15 (Modern Analytical Techniques II: Chromatography & Spectroscopy):** Modern Analytical Techniques II (TLC, GC-MS, HRMS, NMR principles, 13C NMR, Low & High-Res 1H NMR splitting) with 8 interactive theory parts, 6 high-fidelity inline visualizer SVGs (`chromatographySvg`, `chromatogramRfDiagramSvg`, `hrmsSpectrumSvg`, `hplcInstrumentationSvg`, `gcInstrumentationSvg`, `nmrPrinciplesSvg`) split into responsive stacked components for mobile and desktop, and **18 sequentially numbered worked examples** ordered progressively from direct calculations to advanced structure deduction climax (covering TLC/paper Rf calculations, HPLC vs column mechanics, GC retention variables, HRMS mass calculations & spectrum decoding, combined GC-MS/HPLC-MS applications, 13C NMR peak counts, low-res 1H NMR isomer differentiation, high-res 1H NMR n+1 splitting, structural deduction for C4H8O2 isomers, and symmetrical proton splitting rules).
