...
- **Granular Lesson Tree & Syllabus Alignment:** Maps 56 unique lessons for CIE tracks and 21 lessons (across 6 units) for Edexcel, aligned 100% with the official Pearson Edexcel IAL syllabus (Topics 11 to 20). Fully completed interactive curricula for:
  ...
  - **Edexcel Unit 4 Topics 11-15:** Kinetics (fully restructured Lesson 1 "Rate Equations & Reaction Orders"), Entropy & Energetics (Lesson 2 fully standardized across 7 interactive parts: formatted 10 worked examples in `[!EXAMPLE]` and `[!TIP]` containers with `🔑 Step-by-step Solution:`, audited 34 callouts with thematic emojis, fixed KaTeX subscript parentheses collisions on dissolution cycle formulas, and audited 20 quiz questions with `lessonNum: 2` and bullet-pointed explanations), Chemical Equilibria, Acid-Base Equilibria (including Brønsted-Lowry, Ka/pKa/Kw calculations, salt hydrolysis, buffer solutions, Henderson-Hasselbalch equation, titration curves & indicators, and pKa determination from pH curves), and Carbonyls & Chirality.
