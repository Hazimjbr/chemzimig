...
- **Defensive Profile Updates (Fix API 500):** Converted strict Firestore `.update()` calls to safe `.set(..., { merge: true })` inside the profile update API. This prevents HTTP 500 server crashes when users/students update profile settings (such as image or lastStudiedLesson) before their Firestore document has been explicitly created, backed by detailed diagnostic server-side logging.
...
- **Granular Lesson Tree & Syllabus Alignment:** Maps 56 unique lessons for CIE tracks and 21 lessons (across 6 units) for Edexcel, aligned 100% with the official Pearson Edexcel IAL syllabus (Topics 11 to 20). Fully completed interactive curricula for:
  ...
  - **Edexcel Unit 4 Topics 11-15:** Kinetics (fully restructured Lesson 1 "Rate Equations & Reaction Orders" to separate the definition of reaction rate from the rate equation; integrated 3 new worked examples covering stoichiometry rate calculations for $\text{N}_2\text{O}_4$ decomposition, rate/reactant calculations for $\text{Rate} = k[\text{A}]^2[\text{B}]$, and conceptual concentration adjustments; added a responsive, custom 3-column SVG Kinetics Graph Dashboard summarizing Zero, First, and Second-order kinetics curves, fully protected by the `svgToken` utility to guarantee correct client rendering; separated physical monitoring methods—Mass Change, Disappearing Cross, and remaining continuous monitoring methods—into distinct Callout containers with dedicated SVG diagrams to optimize readability and formatting stability; integrated custom first-order constant half-life decay graph `halfLifeSvg` with detailed explanation; added classical activation energy profile `activationEnergyProfileSvg` with user-tuned coordinates and Ea notation, shifted the uncatalyzed activation energy arrow to x=250 and the catalyzed arrow to x=300, and added horizontal dashed transition state level lines; integrated custom `autocatalysisCurveSvg` Reactant Concentration vs. Time graph; updated the custom table parser in the main curriculum page to support tables nested within blockquotes at any level; simplified LaTeX chemical formulas inside table cells to use clean inline math or Unicode symbols to bypass markdown backslash-stripping; enriched homogeneous, heterogeneous, and autocatalysis content in Part 5), Entropy & Energetics, Chemical Equilibria, Acid-Base Equilibria (including Brønsted-Lowry, Ka/pKa/Kw calculations, salt hydrolysis, buffer solutions, Henderson-Hasselbalch equation, titration curves & indicators, and pKa determination from pH curves), and Carbonyls & Chirality.
  ...
  - **Exam Practice Banks:** Fully integrated (38 new MCQs) with topic tagging, lesson-number filtering, and unicode-safe KaTeX rendering.
