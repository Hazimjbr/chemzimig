...
interface ShapeData {
    ...
    color: string;
    description: string;
    atomPositions: { x: number; y: number; label: string; isLone?: boolean }[];
...
const SHAPES: Record<string, ShapeData> = {
    linear: {
        ...
        atomPositions: [
            { x: 250, y: 180, label: 'Be' },
            { x: 100, y: 180, label: 'Cl' },
            { x: 400, y: 180, label: 'Cl' },
        ],
    ...
    trigonal_pyramidal: {
        ...
        atomPositions: [
            { x: 250, y: 175, label: 'N' },
            { x: 250, y: 238, label: 'e⁻e⁻', isLone: true },
            { x: 250, y: 90, label: 'H' },
            { x: 148, y: 235, label: 'H' },
            { x: 352, y: 235, label: 'H' },
        ],
    ...
    bent: {
        ...
        atomPositions: [
            { x: 250, y: 175, label: 'O' },
            { x: 185, y: 120, label: 'e⁻e⁻', isLone: true },
            { x: 315, y: 120, label: 'e⁻e⁻', isLone: true },
            { x: 148, y: 240, label: 'H' },
            { x: 352, y: 240, label: 'H' },
        ],
    ...
    trigonal_bipyramidal: {
        ...
        atomPositions: [
            { x: 250, y: 175, label: 'P' },
            { x: 250, y: 75, label: 'Cl' },
            { x: 250, y: 275, label: 'Cl' },
            { x: 135, y: 215, label: 'Cl' },
            { x: 365, y: 215, label: 'Cl' },
            { x: 250, y: 145, label: 'Cl' },
        ],
    ...
    seesaw: {
        ...
        atomPositions: [
            ...
            { x: 142, y: 220, label: 'F' },
            { x: 358, y: 220, label: 'F' },
            { x: 250, y: 138, label: 'e⁻e⁻', isLone: true },
        ],
        bonds: [
    ...
    square_planar: {
        ...
        atomPositions: [
            { x: 250, y: 175, label: 'Xe' },
            { x: 250, y: 85,  label: 'e⁻e⁻', isLone: true },
            { x: 250, y: 265, label: 'e⁻e⁻', isLone: true },
            { x: 145, y: 175, label: 'F' },
            { x: 355, y: 175, label: 'F' },
            { x: 175, y: 105, label: 'F' },
            { x: 325, y: 105, label: 'F' },
...
export default function MolecularShapeSimulator() {
    ...
    return (
        <div className="flex-shrink-0 rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/90 to-slate-800/60 p-3 sm:p-5 shadow-xl w-full">
            {/* Header */}
            <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                ...
                <div className="min-w-0">
                    ...
                    <p className="text-[10px] sm:text-xs text-slate-400 hidden xs:block">Interactive electron-pair repulsion visualiser</p>
                </div>
            </div>

            {/* Shape Selector — horizontal scroll on mobile, wraps on larger screens */}
                ...
                <div className="flex gap-1.5 sm:flex-wrap sm:gap-1.5" style={{ minWidth: 'max-content' }}>
                    {SHAPE_KEYS.map((key) => (
                        <button
                            ...
                            className="flex-shrink-0 px-2 sm:px-2.5 py-1 rounded-lg text-[10px] sm:text-[11px] font-medium transition-all duration-200 border whitespace-nowrap"
            ...
            {/* Main Content: SVG + Info — stacked by default, side-by-side on lg screens */}
            <div className="flex flex-col lg:flex-row gap-4 items-center lg:items-start w-full">
                {/* SVG Diagram — Centered and constrained on mobile, auto width on desktop */}
                <div className="w-full max-w-[420px] lg:w-[360px] lg:max-w-none flex-shrink-0 rounded-xl overflow-hidden border border-white/10 bg-gradient-to-br from-slate-950 to-slate-900">
                    <svg
                    ...
                    >
                        ...
                        {/* Bond angle arc hint */}
                        <circle
                            cx={shape.atomPositions[0].x}
                            cy={shape.atomPositions[0].y}
                            r={42}
                            fill="none"
                            stroke={shape.color}
                            strokeWidth="1"
                        ...
                        {shape.bonds.map((bond, i) => {
                            const from = shape.atomPositions[bond.from];
                            const to = shape.atomPositions[bond.to];
                            return (
                                <line
                                    key={i}
                                    x1={from.x} y1={from.y}
                        ...
                        {/* Atoms & lone pair blobs */}
                        {shape.atomPositions.map((atom, i) => {
                            const isCenter = i === 0;
                            const radius = isCenter ? 22 : atom.isLone ? 16 : 18;
                            return (
                                <g
                                    key={i}
                                    filter={
                                ...
                                >
                                    <circle
                                        ...
                                        opacity={atom.isLone ? 0.8 : 1}
                                    />
                                    <text
                                        x={atom.x}
                                        y={atom.y + 4}
                                        textAnchor="middle"
                                        fontSize={isCenter ? 13 : atom.isLone ? 10 : 12}
